...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = [`${client.guilds.size} Server`,'w.help','Type w.help',`${client.users.size} Members`,'w.inv','By: M7MEDYT'];
    var i = -1;
    var j = 0;
...
『W.setwelcomer <text channel name> لاختيار روم للترحيب』
    ...
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .addField('``My ID``' , `[ ${client.user.id} ]` , true)
			      ...
			      .setFooter('By | M7MEDYT')
    })
}
});
 client.on('message', message => {
              if (!message.channel.guild) return;
      if(message.content =='W.count')
...
client.on('message', message => {
    if (message.content.startsWith("W.trans")) {
    ...
    if (!language) {
        return message.reply(`Please supply valid agruments.\n**Example** \`W.trans [text] to [language]\``);
    }
...
client.on('message', message => {
    if (message.content.startsWith("W.bans")) {
...
client.on('message', message => {
  ...
  .setColor("#9B59B6")
  .addField(" ** :gear: Server Support :gear: **" , "  **https://discord.gg/V485h6C")
...
client.on('message', message => {
          let args = message.content.split(' ').slice(1);
   if(message.content.split(' ')[0] == 'W.color'){
...
.addField('Type W.mypoints', 'To Show ur Points' , true)
.setFooter(message.author.username, message.author.avatarURL)
...
'*** انا لست الحوت الازرق كما يدعون ***',
 ...
 client.on('message', message => {
 if (message.content.startsWith('W.مريم')) {
...
client.on('message', message => {
 if (message.content.startsWith("W.عقاب")) {
              if(!message.channel.guild) return message.reply('** This command only for servers**');
var embed = new Discord.RichEmbed()
.setColor('RANDOM')
 .setThumbnail(message.author.avatarURL) 
...
${prefix}queue ⇏ لمعرفة قآئمة التشغيل
 ...
 client.on('message', message => {
            if(!message.channel.guild) return;
let args = message.content.split(' ').slice(1).join(' ');
if (message.content.startsWith('W.users')){
...
if (message.author.id !== '306464381775118336') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
client.user.setGame(argresult);
